...
class AES128GCM : public EncryptionService
{
    static constexpr size_t KEY_SIZE = 16;   // 128 бит для AES-128
...
public:
    /**
     * @brief Шифрует PlaintextEntry с использованием AES-128-GCM
     * @param key Ключ шифрования (должен быть 16 байт)
     ...
     * 3. Шифрование JSON-строки с помощью AES-128-GCM
    ...
    {
        // 1. Сериализуем PlaintextEntry в JSON
        json json_data;
        PlaintextEntry::to_json(json_data, entry);
        std::string plaintext = json_data.dump();

        // 2. Генерация случайного nonce
        ...
        try
        {
            // 5. Инициализация контекста для шифрования с алгоритмом AES-128-GCM
            ...
            // 10. Обрезаем шифротекст до реального размера
    ...
    PlaintextEntry decrypt(const std::vector<uint8_t>& encrypted_package, const KeyManager::KeyData& key) override
    {
        ...
        try
        {
            // 4. Инициализация контекста для дешифрования с алгоритмом AES-128-GCM
